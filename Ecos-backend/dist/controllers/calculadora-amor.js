...
Resposta da especialista em amor (certifique-se de completar TODA sua análise antes de terminar):`;
                ...
                console.log(`✅ Análise de compatibilidade gerada com sucesso com ${usedModel} (${text.length} caracteres)`);
    ...
    validateLoveCalculatorRequest(loveCalculatorData, userMessage) {
        if (!loveCalculatorData) {
            const error = new Error("Dados da especialista em amor são obrigatórios");
        ...
        if (!userMessage ||
            ...
            userMessage.trim() === "") {
            const error = new Error("Mensagem do usuário é obrigatória");
            error.statusCode = 400;
            error.code = "MISSING_USER_MESSAGE";
            throw error;
        }
        if (userMessage.length > 1200) {
            const error = new Error("A mensagem é muito longa (máximo 1200 caracteres)");
            error.statusCode = 400;
