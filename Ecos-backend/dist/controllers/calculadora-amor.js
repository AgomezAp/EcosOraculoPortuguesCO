...
Resposta da especialista em amor (EM PORTUGUÊS DE PORTUGAL):`;
                console.log(`A gerar análise de compatibilidade amorosa (${shouldGiveFullResponse ? "COMPLETA" : "PARCIAL"})...`);
                let text = "";
                let usedModel = "";
                let allModelErrors = [];
                ...
                console.log(`✅ Análise gerada (${shouldGiveFullResponse ? "COMPLETA" : "PARCIAL"}) com ${usedModel} (${finalResponse.length} caracteres)`);
            ...
            try {
                res.json({
                    ...
                    loveExpert: {
                        ...
                        description: "Especialista em numerologia do amor especializada em analisar a compatibilidade entre casais",
                        services: [
    ...
    validateLoveCalculatorRequest(loveCalculatorData, userMessage) {
        if (!loveCalculatorData) {
            const error = new Error("Dados do especialista em amor necessários");
        ...
        if (userMessage.length > 1200) {
            const error = new Error("A mensagem é demasiado longa (máximo 1200 caracteres)");
            error.statusCode = 400;
