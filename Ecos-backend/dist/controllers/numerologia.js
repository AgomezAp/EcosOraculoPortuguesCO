    ...
    constructor() {
        ...
        this.chatWithNumerologist = (req, res) => __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                console.log(`📊 Numerologia - Contagem de mensagens: ${messageCount}, Premium: ${isPremiumUser}, Resposta completa: ${shouldGiveFullResponse}`);
...
- Se mencionas que vais calcular números, ${shouldGiveFullResponse
...
Resposta da numeróloga (EM PORTUGUÊS DE PORTUGAL):`;
                console.log(`A gerar leitura numerológica (${shouldGiveFullResponse ? "COMPLETA" : "PARCIAL"})...`);
                let text = "";
                let usedModel = "";
                let allModelErrors = [];
                ...
                console.log(`✅ Leitura numerológica gerada (${shouldGiveFullResponse ? "COMPLETA" : "PARCIAL"}) com ${usedModel} (${finalResponse.length} caracteres)`);
            ...
            try {
                res.json({
                    ...
                    numerologist: {
                        ...
                        description: "Numeróloga ancestral especializada em decifrar os mistérios dos números e a sua influência na vida",
                        services: [
...
Lembra-te: És uma guia numerológica sábia mas ACESSÍVEL que ${isFullResponse
            ? "revela os segredos dos números de forma completa"
            : "intriga sobre os mistérios numéricos que detetaste"}. Fala como uma amiga curiosa e entusiasta. ${isFullResponse
    ...
    validateNumerologyRequest(numerologyData, userMessage) {
        if (!numerologyData) {
            const error = new Error("Dados da numeróloga necessários");
    ...
    handleError(error, res) {
        ...
        let statusCode = 500;
        let errorMessage = "As energias numéricas estão temporariamente perturbadas. Por favor, tenta novamente.";
